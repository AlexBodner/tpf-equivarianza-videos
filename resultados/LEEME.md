...
| `../videos_t2v_65_por_escenario/` | generación **sólo desde texto** a 65 cuadros en los checkpoints elegidos, 3 escenarios × 2 semillas × 2 brazos. Sin métricas: t2v no tiene clip real apareado, así que no hay error de velocidad contra qué medir |
| `evaluaciones/angulo_sorteado_11_45__*.json` | **reevaluación de la equivarianza con el ángulo sorteado por clip** en [11°, 44°] en vez de 45° fijo, uno por escenario. Control 125 contra física 875, 15 clips held-out cada uno (00520–00534), apareado. El ángulo de cada clip queda escrito en `per_clip.rot_grados` y sale del CRC32 del nombre del clip, así que es el mismo en los dos brazos y reproducible. **El error de velocidad de estos JSON no se usa**: no depende del ángulo y está medido sobre menos clips que el test principal |
